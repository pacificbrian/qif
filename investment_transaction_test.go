...
func TestCheckPrice(t *testing.T) {
	...
	err := tx.parseInvestmentTransactionField("Q"+price, Config{})
	require.NoError(t, err)

	priceDecimal, _ := decimal.NewFromString(price)
	assert.Equal(t, priceDecimal, tx.Price())
}
...
func TestCheckShares(t *testing.T) {
	...
	err := tx.parseInvestmentTransactionField("I"+shares, Config{})
	require.NoError(t, err)

	sharesDecimal, _ := decimal.NewFromString(shares)
	assert.Equal(t, sharesDecimal, tx.Shares())
}
